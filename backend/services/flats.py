...
def create_flat(dto: CreateFlatDto) -> Flat:
    ...
    with db.get_conn() as conn:
        existing = db.fetchone(conn, "SELECT id FROM flat WHERE id = %s", (dto.id,))
        if existing:
        ...
        conn.commit()
        row = db.fetchone(conn, "SELECT * FROM flat WHERE id = %s", (dto.id,))
    return _row_to_flat(row)


def list_flats() -> List[Flat]:
    with db.get_conn() as conn:
        rows = db.fetchall(conn, "SELECT * FROM flat WHERE is_deleted = FALSE")
    return [_row_to_flat(r) for r in rows]


def get_flat(flat_id: str) -> Flat:
    with db.get_conn() as conn:
        row = db.fetchone(conn, "SELECT * FROM flat WHERE id = %s AND is_deleted = FALSE", (flat_id,))
    if not row:
...
def update_flat(flat_id: str, dto: UpdateFlatDto) -> Flat:
    ...
    with db.get_conn() as conn:
        row = db.fetchone(conn, "SELECT id FROM flat WHERE id = %s AND is_deleted = FALSE", (flat_id,))
        if not row:
            raise HTTPException(404, "Flat not found")
        db.execute(conn, f"UPDATE flat SET {set_clause} WHERE id = %s", values)
        conn.commit()
        updated = db.fetchone(conn, "SELECT * FROM flat WHERE id = %s", (flat_id,))
...
def delete_flat(flat_id: str) -> None:
    with db.get_conn() as conn:
        ...
        db.execute(
            conn,
            "UPDATE flat SET is_deleted = TRUE, last_updated = %s WHERE id = %s",
            (_now(), flat_id),
        )
        conn.commit()
